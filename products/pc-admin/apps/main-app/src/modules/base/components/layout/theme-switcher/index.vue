...
function handleThemeClick(themeConfig: ThemeConfig) {
  // 优先使用插件API
  if (typeof window !== 'undefined' && (window as any).__PLUGIN_API__?.theme?.changeTheme) {
    (window as any).__PLUGIN_API__.theme.changeTheme(themeConfig);
...
function handleDarkToggle(event?: MouseEvent) {
  ...
  if (typeof window !== 'undefined' && (window as any).__PLUGIN_API__?.theme?.changeTheme) {
    ...
    (window as any).__PLUGIN_API__.theme.changeTheme(currentIsDark ? 'light' : 'dark');
    return;
  }
