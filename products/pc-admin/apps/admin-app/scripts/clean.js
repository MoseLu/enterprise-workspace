  ...
  if (existsSync(assetsDir)) {
    try {
      ...
      if (vendorFiles.length > 1) {
        logger.info(`⚠️  发现多个 vendor 文件: ${vendorFiles.join(', ')}`);
      }
      ...
      for (const vendorFile of vendorFiles) {
        for (const oldHash of oldVendorHashes) {
          if (vendorFile.includes(oldHash)) {
            logger.info(`⚠️  发现旧的 vendor 文件: ${vendorFile}`);
            break;
          }
        }
      }
...
pathsToClean.slice(1).forEach((path) => {
  if (existsSync(path)) {
    ...
    logger.info(`✓ 路径不存在（已干净）: ${path.replace(appDir, '.')}`);
  }
});
