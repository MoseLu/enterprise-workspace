  ...
  if (isStandalone) {
    ...
    try {
      ...
      if (appLoadingService) {
        appLoadingService.show(tSync('common.system.quality_module'));
      }
    } catch (error) {
      // 静默失败，继续执行
      if (import.meta.env.DEV) {
        console.warn(`[quality-app] ${tSync('common.error.cannot_display_loading')}:`, error);
      }
    }
  }
...
async function update(props: QiankunProps) {
  if (context) {
    updateQualityApp(context, props);
  }
}
