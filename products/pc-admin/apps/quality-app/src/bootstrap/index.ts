...
import { createQualityRouter, setupRouter, setupStore, setupI18n, setupUI } from './core';

// 导出类型（保持向后兼容）
export type QualityAppContext = SubAppContext;

const QUALITY_APP_ID = 'quality';
const QUALITY_BASE_PATH = '/quality';
const QUALITY_DOMAIN_CACHE_PATH = '@utils/domain-cache';

// 子应用配置（标准化模板）
const subAppOptions: SubAppOptions = {
  appId: QUALITY_APP_ID,
  basePath: QUALITY_BASE_PATH,
  domainCachePath: QUALITY_DOMAIN_CACHE_PATH,
  AppComponent: App,
  createRouter: createQualityRouter,
  setupRouter,
  setupStore,
  setupI18n,
  setupUI,
...
export const createQualityApp = async (props: QiankunProps = {}): Promise<QualityAppContext> => {
...
export const mountQualityApp = async (context: QualityAppContext, props: QiankunProps = {}) => {
  // 使用标准化的 mountSubApp
  await mountSubApp(context, subAppOptions, props);

  // 设置路由同步、事件桥接等
  setupRouteSync(context, QUALITY_APP_ID, QUALITY_BASE_PATH);
  setupHostLocationBridge(context, QUALITY_APP_ID, QUALITY_BASE_PATH);
  setupEventBridge(context);
  ensureCleanUrl(context);

  // 设置退出登录函数（在应用挂载后设置，确保 router 和 i18n 已初始化）
  (window as any).__APP_LOGOUT__ = createLogoutFunction(context, QUALITY_APP_ID);
};

export const updateQualityApp = (context: QualityAppContext, props: QiankunProps) => {
  updateSubApp(context, props);
};

export const unmountQualityApp = async (context: QualityAppContext, props: QiankunProps = {}) => {
  // 清理全局资源（避免内存泄漏）
