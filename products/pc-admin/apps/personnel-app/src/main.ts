...
const render = async (props: QiankunProps = {}) => {
  ...
  try {
    ...
    if (context) {
      try {
        await unmountPersonnelApp(context);
      } catch (error) {
        // 卸载失败不影响后续流程，但记录错误
        if (import.meta.env.DEV) {
    ...
    // 创建新实例
    context = await createPersonnelApp(props);
    await mountPersonnelApp(context, props);

    // 关键：应用挂载完成后，移除 Loading 并清理 sessionStorage 标记
    if (isStandalone && appLoadingService) {
      // 隐藏应用级 loading
      try {
        appLoadingService.hide('人事模块');
      } catch (error) {
        // 静默失败
      }
    }
...
async function unmount(props: QiankunProps = {}) {
  ...
  if (context) {
    try {
      await unmountPersonnelApp(context, props);
    } catch (error) {
      // 卸载失败不影响后续流程
      if (import.meta.env.DEV) {
...
async function update(props: QiankunProps) {
  if (context) {
    updatePersonnelApp(context, props);
  }
}
