...
export function useRetry(config: RetryConfig = {}) {
    ...
    for (let attempt = 0; attempt <= operationConfig.maxRetries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          ...
          console.info('common.retry.request_retry', `${attempt}/${operationConfig.maxRetries}`, 'common.retry.delay', `${delay}ms`);
  ...
  function getStatus() {
    return {
      ...
      isRetrying: state.value.isRetrying,
      lastError: state.value.lastError,
      nextRetryDelay: state.value.nextRetryDelay
    };
  }
