...
// 导出类型（保持向后兼容）
export type FinanceAppContext = SubAppContext;

const FINANCE_APP_ID = 'finance';
const FINANCE_BASE_PATH = '/finance';
const FINANCE_DOMAIN_CACHE_PATH = '@utils/domain-cache';

// 子应用配置（标准化模板）
const subAppOptions: SubAppOptions = {
  appId: FINANCE_APP_ID,
  basePath: FINANCE_BASE_PATH,
  domainCachePath: FINANCE_DOMAIN_CACHE_PATH,
  AppComponent: App,
  createRouter: createFinanceRouter,
  setupRouter,
  setupStore,
  setupI18n,
  setupUI,
...
export const createFinanceApp = async (props: QiankunProps = {}): Promise<FinanceAppContext> => {
  ...
  if (isStandalone || isUsingLayoutApp) {
    ...
    try {
      ...
      console.warn('[finance-app] Failed to import domain-cache module:', error);
    }
...
export const mountFinanceApp = async (context: FinanceAppContext, props: QiankunProps = {}) => {
  ...
  // 设置路由同步、事件桥接等
  setupRouteSync(context, FINANCE_APP_ID, FINANCE_BASE_PATH);
  setupHostLocationBridge(context, FINANCE_APP_ID, FINANCE_BASE_PATH);
  setupEventBridge(context);
  ensureCleanUrl(context);

  // 设置退出登录函数（在应用挂载后设置，确保 router 和 i18n 已初始化）
  (window as any).__APP_LOGOUT__ = createLogoutFunction(context, FINANCE_APP_ID);
};
...
export const unmountFinanceApp = async (context: FinanceAppContext, props: QiankunProps = {}) => {
  // 清理全局资源（避免内存泄漏）
