...
export function useAvatarRhythm(containerRef: Ref<HTMLElement | null>) {
  ...
  function addShakeEffect() {
    ...
    const avatarElement = containerRef.value.querySelector('.btc-avatar__img') as HTMLElement;
      ...
      setTimeout(() => {
        if (bars[index]) {
          ...
          bar.style.background = `linear-gradient(to top,
            ...
            ${randomColor},
            hsla(${(currentHue + 60) % 360}, 90%, 70%, 0.8))`;

          // 暗色模式适配
          if (document.documentElement.classList.contains('dark')) {
            bar.style.background = `linear-gradient(to top,
              hsla(${currentHue}, 70%, 60%, 0.9),
              ${randomColor},
              hsla(${(currentHue + 60) % 360}, 80%, 80%, 0.8))`;
          }
        }
      }, 200);
    });
