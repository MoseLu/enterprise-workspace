...
export function useTabs({ config, Form }: { config: BtcFormConfig; Form: any }) {
  // 选中
  ...
  function getItem(value: any) {
    return list.value.find((e: any) => e.value === value);
  }
  ...
  // 查找分组
  function toGroup(opts: { config: BtcFormConfig; prop: string; refs: any }) {
    if (active.value) {
      let name;
  ...
  // 获取参数
  function get() {
    return config.items.find((e: any) => e.type === 'tabs');
  }
  ...
  function mergeProp(item: any) {
    ...
    if (d && d.props) {
      ...
      if (mergeProp) {
        const t = labels.find((e: any) => e.value === item.group);
