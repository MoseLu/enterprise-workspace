...
/**
 * 动态获取子应用路径前缀列表
 * 从 app-env.config.ts 中获取所有子应用配置，排除主应用，转换为路径前缀
 */
...
export async function handleCrossAppRedirect(redirectPath: string, _router?: any): Promise<boolean> {
  ...
  if (redirectPath.startsWith('http://') || redirectPath.startsWith('https://')) {
    ...
    try {
      ...
      if (isAllowed) {
        // 关键：在开发环境下，如果 URL 是主应用地址，应该返回 false
        // 让 Vue Router 在主应用内处理路由，而不是使用 window.location.href 导致页面刷新
        if (env === 'development') {
          const mainAppOrigin = getMainAppOrigin();
          try {
            const mainUrl = new URL(mainAppOrigin);
            if (url.hostname === mainUrl.hostname && url.port === mainUrl.port) {
              return false; // 让 Vue Router 处理，避免页面刷新
            }
          } catch (e) {
            // 如果解析失败，继续后续处理
          }
  ...
  try {
    ...
    // 这样可以确保路由匹配和认证检查都能正确执行，避免使用 window.location.href 导致页面刷新
    if (env === 'development') {
    ...
    if (env === 'test') {
      ...
      // 其他环境（poc、sit、uat等）：假设在同一域名下，使用路径前缀（主应用路由）
      return false;
    }
    
    // 使用window.location进行跨应用跳转（仅在非开发环境）
