...
async function confirmScripts() {
  ...
  for (const [categoryName, categoryData] of categories) {
    ...
    // 询问是否批量处理
    const { batchAction } = await prompts({
      type: 'select',
      name: 'batchAction',
      message: `如何处理 ${categoryName} 的 ${scripts.length} 个脚本？`,
      choices: [
        { title: '逐个确认', value: 'one-by-one' },
        { title: '全部保留', value: 'all-keep' },
        { title: '全部归档', value: 'all-archive' },
        { title: '跳过此分类', value: 'skip' },
      ],
      initial: 0,
    });
    
    if (!batchAction || batchAction === 'skip') {
      console.log(`   ⏭️  跳过 ${categoryName}`);
      continue;
    }
    
    if (batchAction === 'all-keep') {
