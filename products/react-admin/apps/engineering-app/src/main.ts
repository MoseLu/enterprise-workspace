...
import {
  createEngineeringApp,
  mountEngineeringApp,
  unmountEngineeringApp,
  updateEngineeringApp,
} from './bootstrap-subapp';
import type { EngineeringAppContext } from './bootstrap-subapp';
...
const render = async (props: QiankunProps = {}) => {
  ...
  if (isStandalone) {
    ...
    try {
      ...
      if (appLoadingService) {
        appLoadingService.show(tSync('domain.type.engineering') || '工程模块');
      }
    } catch (error) {
      // 静默失败，继续执行
  ...
  try {
    ...
    if (isStandalone && appLoadingService) {
      // 隐藏应用级 loading
      try {
        appLoadingService.hide(tSync('domain.type.engineering') || '工程模块');
      } catch (error) {
        // 静默失败
      }
    }
    ...
    logger.error('[engineering-app] 渲染失败:', error);
...
if (qiankunWindow.__POWERED_BY_QIANKUN__) {
  renderWithQiankun({
    ...
    async update(props: QiankunProps) {
      if (context) {
        updateEngineeringApp(context, props);
      }
    },
    unmount,
  });
...
// 独立运行（非 qiankun 环境）
// 注意：engineering-app 作为子应用，独立运行时应该直接渲染（不加载 layout-app）
// 因为 engineering-app 现在已经是子应用，不再需要 layout-app 的支持
...
if (shouldRunStandalone()) {
  ...
  render().catch((error) => {
    logger.error('[engineering-app] 独立运行失败:', error);
  });
}
