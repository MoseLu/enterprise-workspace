...
import { createEngineeringRouter, setupRouter, setupStore, setupI18n, setupUI } from './bootstrap/core';
...
export const createEngineeringApp = async (props: QiankunProps = {}): Promise<EngineeringAppContext> => {
  ...
  const { setEngineeringAppProps } = await import('./bootstrap/core/i18n');
  setEngineeringAppProps(props);
    ...
    try {
      ...
      if (messages && (messages['zh-CN'] || messages['en-US'])) {
        ...
        props.setGlobalState({
          subAppI18nMessages: {
            [ENGINEERING_APP_ID]: messages,
          },
        });
      }
    } catch (error) {
      // 注意：不能在非 setup 函数中调用 useI18n()，使用 tSync 或直接使用字符串
      console.warn('[engineering-app] 发送国际化消息失败:', error);
    }
  }
