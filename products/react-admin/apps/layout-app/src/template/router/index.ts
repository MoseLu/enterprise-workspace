...
import { get{{APP_NAME_PASCAL}}Routes } from './routes/app';
...
export const create{{APP_NAME_PASCAL}}Router = (): Router => {
  // 动态获取路由配置，确保在运行时正确检测是否使用 layout-app
  const routes = get{{APP_NAME_PASCAL}}Routes();
...
// 导出路由配置获取函数和路由常量
export { get{{APP_NAME_PASCAL}}Routes, {{APP_NAME}}Routes } from './routes/app';
