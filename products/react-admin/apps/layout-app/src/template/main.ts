...
import {
  create{{APP_NAME_PASCAL}}App,
  mount{{APP_NAME_PASCAL}}App,
  unmount{{APP_NAME_PASCAL}}App,
  update{{APP_NAME_PASCAL}}App,
} from './bootstrap';
import type { {{APP_NAME_PASCAL}}AppContext } from './bootstrap';
import { loadSharedResourcesFromLayoutApp } from '@btc/shared-utils/cdn/load-shared-resources';

let context: {{APP_NAME_PASCAL}}AppContext | null = null;
...
const render = async (props: QiankunProps = {}) => {
  ...
  if (isStandalone) {
    ...
    try {
      ...
      if (appLoadingService) {
        appLoadingService.show('{{APP_TITLE}}');
      }
    } catch (error) {
      // 静默失败，继续执行
      if (import.meta.env.DEV) {
        console.warn('[{{APP_NAME}}-app] 无法显示应用级 loading:', error);
      }
    }
  }
  ...
  try {
    ...
    if (context) {
      try {
        await unmount{{APP_NAME_PASCAL}}App(context);
      } catch (error) {
        // 卸载失败不影响后续流程，但记录错误
        if (import.meta.env.DEV) {
          console.warn('[{{APP_NAME}}-app] 卸载前一个实例失败:', error);
        }
      } finally {
        context = null;
      }
    ...
    // 创建新实例
    context = await create{{APP_NAME_PASCAL}}App(props);
    await mount{{APP_NAME_PASCAL}}App(context, props);

    // 关键：应用挂载完成后，移除 Loading 并清理 sessionStorage 标记
    if (isStandalone && appLoadingService) {
      // 隐藏应用级 loading
      try {
        appLoadingService.hide('{{APP_TITLE}}');
      } catch (error) {
        // 静默失败
      }
    }
    ...
    try {
      await loadSharedResourcesFromLayoutApp({
        onProgress: (_loaded: number, _total: number) => {
          if (import.meta.env.DEV) {
            console.info(`[{{APP_NAME}}-app] 加载共享资源进度: ${_loaded}/${_total}`);
          }
        },
      });
    } catch (error) {
      console.warn('[{{APP_NAME}}-app] 加载共享资源失败，继续使用本地资源:', error);
...
async function unmount(props: QiankunProps = {}) {
  ...
  if (context) {
    try {
      await unmount{{APP_NAME_PASCAL}}App(context, props);
    } catch (error) {
      // 卸载失败不影响后续流程
      if (import.meta.env.DEV) {
        console.warn('[{{APP_NAME}}-app] 卸载失败:', error);
      }
    } finally {
      context = null;
    }
...
async function update(props: QiankunProps) {
  if (context) {
    update{{APP_NAME_PASCAL}}App(context, props);
  }
}
            ...
            waitForViewport().then((viewport) => {
              if (viewport) {
                ...
                if (import.meta.env.PROD) {
                  ...
                  console.warn('[{{APP_NAME}}-app] 找到 #subapp-viewport，准备挂载', {
