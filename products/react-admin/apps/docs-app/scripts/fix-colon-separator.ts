...
async function main() {
  ...
  console.info('╔════════════════════════════════════════════════════════╗');
  console.info('║        文档编码修复工具 - 冒号分隔符移除              ║');
  console.info('╚════════════════════════════════════════════════════════╝\n');

  if (dryRun) {
    console.info('🔍 运行模式：演练模式（不会修改文件）\n');
  } else if (testMode) {
    console.info('🧪 运行模式：测试模式（仅处理前5个文件）\n');
  } else {
    console.info('🔧 运行模式：完整修复模式\n');
  }
  ...
  if (testMode) {
    console.info(`🧪 测试模式：仅处理前 ${filesToProcess.length} 个文件：`);
    filesToProcess.forEach((f, i) => console.info(`   ${i + 1}. ${f}`));
    console.info();
  }

  // 处理文件
  console.info('⚙️  开始处理文件...\n');
  ...
  for (const file of filesToProcess) {
    ...
    if (result.success && result.colonCount > 0) {
      ...
      console.info(`   移除 ${result.colonCount} 个冒号分隔符`);
      console.info(`   ${result.charsBefore} → ${result.charsAfter} 字符 (减少 ${reduction}%)\n`);
    } else if (result.error?.includes('clean')) {
      console.info(`✨ ${result.file} (已是正常文件)\n`);
    } else if (!result.success) {
      console.info(`❌ ${result.file}`);
      console.info(`   错误: ${result.error}\n`);
    }
  }
  ...
  console.info(`📊 统计信息：`);
  console.info(`   - 总文件数：${filesToProcess.length}`);
  console.info(`   - 需要修复：${successResults.length}`);
  console.info(`   - 已是正常：${cleanResults.length}`);
  console.info(`   - 修复失败：${failedResults.length}`);
  console.info();

  if (successResults.length > 0) {
    console.info(`🔧 修复详情：`);
    console.info(`   - 移除冒号总数：${totalColonsRemoved.toLocaleString()}`);
    console.info(`   - 修复前字符数：${totalCharsBefore.toLocaleString()}`);
    console.info(`   - 修复后字符数：${totalCharsAfter.toLocaleString()}`);
    console.info(`   - 减少字符数：${(totalCharsBefore - totalCharsAfter).toLocaleString()}`);
    console.info();
  }
  ...
  console.info(`📋 详细报告已保存到: ${path.basename(reportPath)}\n`);

  // 最终提示
  if (dryRun) {
    console.info('💡 提示：这是演练模式，文件未被修改');
    console.info('   运行 pnpm tsx scripts/fix-colon-separator.ts 执行实际修复\n');
  } else if (testMode) {
    console.info('💡 提示：这是测试模式，仅处理了前5个文件');
    console.info('   如果结果正确，运行不带 --test 参数的命令处理所有文件\n');
  } else {
    console.info('✅ 修复完成！请验证文档是否正常：');
    console.info('   1. 检查关键文件内容');
    console.info('   2. 运行 pnpm dev 启动 VitePress');
    console.info('   3. 检查导航和搜索功能\n');
  }
}
