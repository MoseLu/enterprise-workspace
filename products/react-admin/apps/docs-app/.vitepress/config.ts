// 注意：在 VitePress 配置文件中，使用 @enterprise-workspace/frontend 作为共享依赖
...
// 从 apps 目录的相对路径，指向 react-admin 的 configs
import { getViteAppConfig } from '../../apps/admin-app/vite.config';
...
`(function(){
  ...
  vite: {
    plugins: [
      ...
      // localesStaticPlugin 移除，文档使用 VitePress 原生国际化
    ],
    ...
    resolve: {
      alias: {
        // 使用 @enterprise-workspace/frontend 作为共享依赖
        '@enterprise-workspace/frontend': fileURLToPath(new URL('../../../../common/frontend', import.meta.url)),
      }
    },
    ...
    // SSR 配置
    ssr: {
      noExternal: ['element-plus', '@enterprise-workspace/frontend', 'vue', 'file-saver'],
