...
// 从 config.ts 读取配置
const { columns: baseColumns } = usePageColumns('warehouse.material.list');
const { formItems } = usePageForms('warehouse.material.list');
const pageConfig = getPageConfigFull('warehouse.material.list');

// 使用 config.ts 中定义的服务
const materialService = usePageService('warehouse.material.list', 'material');
